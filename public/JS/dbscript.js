  ...
  try {
    ...
    snapshot.forEach(function (childSnapshot) {
      ...
      row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-primary" onclick="editMessage('${charId}', '${
        data.name
      }', '${data.emailid}', '${data.msgContent}', '${
        data.imageURL
      }')">Edit</button>
...
function editMessage(charId, name, emailid, msgContent, imageURL) {
  ...
  if (imageURL) {
    document.getElementById(
      "existingImageLink"
    ).innerHTML = `<a href="${imageURL}" target="_blank">View Current Image</a>`;
  }
  scrollToTop();
}
    ...
    try {
      ...
      snapshot.forEach(function (childSnapshot) {
        ...
        if (
          ...
          data.emailid.toLowerCase().includes(searchInput) ||
          data.msgContent.toLowerCase().includes(searchInput)
        ) {
          var row = document.createElement("tr");
          row.innerHTML = `
                        <td>${data.name}</td>
                        ...
                        <td>${data.msgContent}</td>
                        <td>
                            <button class="btn btn-primary" onclick="editMessage('${charId}', '${data.name}', '${data.emailid}', '${data.msgContent}')">Edit</button>
                            <button class="btn btn-danger" onclick="deleteMessage('${charId}')">Delete</button>
                        </td>
                    `;
          document.getElementById("characterTableBody").appendChild(row);
      ...
      console.error("An error occurred while searching data:", error);
